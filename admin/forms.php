...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $id=(int)($_POST['id']??0);
    if($op==='delete'&&$id){
        $db->queryF("DELETE FROM ".$db->prefix('xcform_submissions')." WHERE form_id='{$id}'");
...
$forms=xcform_get_forms(); $rows=[];
foreach($forms as $f){
    $fid=(int)$f['form_id']; $fields=json_decode($f['fields']??'[]',true)?:[];
    $rows[]=['form_id'=>$fid,'name'=>$f['name'],'slug'=>$f['slug'],'is_active'=>(int)$f['is_active'],'field_count'=>count($fields),'total_subs'=>xcform_count_submissions($fid),'new_subs'=>xcform_count_submissions($fid,0),'tpl_tag'=>'{xcform slug="'.$f['slug'].'"}'];
}
global $xoopsTpl;
...
xcform_admin_assign_lang('forms');
echo $xoopsTpl->fetch($d.'xcform_admin_forms.tpl');
xoops_cp_footer();
