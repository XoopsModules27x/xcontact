...
$templateMain = 'xcontact_admin_clone.tpl';
...
switch ($op) {
    ...
    default:
        ...
        $clone->setDescription(\_AM_XCONTACT_CLONE_NAME_DSC);
    ...
    case 'submit':
        ...
        if ($clone && \is_dir($dirClone)) {
            \redirect_header('clone.php', 3, \sprintf(\_AM_XCONTACT_CLONE_EXISTS, $clone));
        }
        ...
        $content = \str_replace('Xcontact', \mb_strtolower($clone), $content);
        file_put_contents($dirClone . '/language/english/modinfo.php', $content);

        $msg = '';
        if (\is_dir($GLOBALS['xoops']->path('modules/' . \mb_strtolower($clone)))) {
            $msg .= \sprintf(\_AM_XCONTACT_CLONE_CONGRAT, "<a href='" . \XOOPS_URL . "/modules/system/admin.php?fct=modulesadmin&op=installlist'>" . \ucfirst(\mb_strtolower($clone)) . '</a>') . "<br>\n";
            if (!$logocreated) {
                $msg .= \_AM_XCONTACT_CLONE_IMAGEFAIL;
            }
        } else {
            $msg .= \_AM_XCONTACT_CLONE_FAIL;
        }
