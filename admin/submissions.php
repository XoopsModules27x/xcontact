...
$db=$GLOBALS['xoopsDB']; $tbl_subs=$db->prefix('xcform_submissions');
$op=$_GET['op']??'';
...
if($op==='view'&&isset($_GET['id'])){
    $sub=xcform_get_submission((int)$_GET['id']);
    if(!$sub){redirect_header('submissions.php',2,_AM_XCFORM_SUB_NOT_FOUND);exit;}
    $db->queryF("UPDATE {$tbl_subs} SET status=1 WHERE sub_id='".(int)$sub['sub_id']."'");
    $form=xcform_get_form((int)$sub['form_id']); $fields=$form?(json_decode($form['fields'],true)?:[]):[];
    ...
    xcform_admin_assign_lang('submissions');
    echo $xoopsTpl->fetch($d.'xcform_admin_sub_view.tpl');
    xoops_cp_footer(); exit;
}
$form_id=(int)($_GET['form_id']??0); $form=$form_id?xcform_get_form($form_id):null;
$start=max(0,(int)($_GET['start']??0)); $limit=20;
$subs=$form_id?xcform_get_submissions($form_id,$start,$limit):[]; $total=$form_id?xcform_count_submissions($form_id):0;
...
xcform_admin_assign_lang('submissions');
echo $xoopsTpl->fetch($d.'xcform_admin_submissions.tpl');
xoops_cp_footer();
