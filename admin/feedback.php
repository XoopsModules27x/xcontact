...
$feedback = new \XoopsModules\Xcontact\Common\ModuleFeedback();
...
switch ($op) {
    ...
    case 'send':
        ...
        $fb_content_html = \str_replace(["\r\n", "\n", "\r"], '<br>', $fb_content); //clean line break from dhtmltextarea

        $title       = \constant('_CO_XCONTACT_FB_SEND_FOR') . $GLOBALS['xoopsModule']->getVar('dirname');
        $body        = \constant('_CO_XCONTACT_FB_NAME') . ': ' . $your_name . '<br>';
        $body        .= \constant('_CO_XCONTACT_FB_MAIL') . ': ' . $your_mail . '<br>';
        $body        .= \constant('_CO_XCONTACT_FB_SITE') . ': ' . $your_site . '<br>';
        $body        .= \constant('_CO_XCONTACT_FB_TYPE') . ': ' . $fb_type . '<br><br>';
        $body        .= \constant('_CO_XCONTACT_FB_TYPE_CONTENT') . ':<br>';
        $body        .= $fb_content_html;
        ...
        if ($ret) {
            \redirect_header('index.php', 3, \constant('_CO_XCONTACT_FB_SEND_SUCCESS'));
        }
        ...
        echo '<div style="text-align:center;width: 80%; padding: 10px; border: 2px solid #ff0000; color: #ff0000; margin-right:auto;margin-left:auto;">
            <h3>' . \constant('_CO_XCONTACT_FB_SEND_ERROR') . '</h3>
            </div>';
        $form = $feedback->getFormFeedback();
        $form->display();
